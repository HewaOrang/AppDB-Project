"""Import SQL data into SQLite"""
from db_sqlite import SQLiteDatabase
from pathlib import Path

def import_data():
    """Import data from appdbproj.sql into SQLite"""
    
    # Initialize SQLite database
    db = SQLiteDatabase("appdbproj.db")
    ...
    for company in company_data:
        db.execute_insert_update_delete(
            "INSERT OR IGNORE INTO company (companyID, companyName, industry) VALUES (?, ?, ?)",
            company
        )
    ...
    for attendee in attendee_data:
        db.execute_insert_update_delete(
            "INSERT OR IGNORE INTO attendee (attendeeID, attendeeName, attendeeDOB, attendeeGender, attendeeCompanyID) VALUES (?, ?, ?, ?, ?)",
            attendee
        )
    print("✓ Inserted attendee data (20 attendees)")
    ...
    for room in room_data:
        db.execute_insert_update_delete(
            "INSERT OR IGNORE INTO room (roomID, roomName, capacity) VALUES (?, ?, ?)",
            room
        )
    print("✓ Inserted room data (6 rooms)")
    ...
    for session in session_data:
        db.execute_insert_update_delete(
            "INSERT OR IGNORE INTO session (sessionID, sessionTitle, speakerName, sessionDate, roomID) VALUES (?, ?, ?, ?, ?)",
            session
        )
    print("✓ Inserted session data (12 sessions)")
    ...
    for registration in registration_data:
        db.execute_insert_update_delete(
            "INSERT OR IGNORE INTO registration (registrationID, attendeeID, sessionID, registeredAt) VALUES (?, ?, ?, ?)",
            registration
        )
    print("✓ Inserted registration data (30 registrations)")
    
    # Verify data
    result = db.execute_query("SELECT COUNT(*) as count FROM attendee")
    if result:
        print(f"\n✓ Database ready! Total attendees: {result[0]['count']}")
